  ...
  Future<bool> loadModels(String symbol) async {
    try {
      ...
      for (final horizon in ['4H', '2D', '5D']) {
        final assetKey = 'assets/models/${symbol}_$horizon.json';
        final localPath = '$_modelsPath/${symbol}_$horizon.json';
        
        try {
          // 1. Try local storage first (Imported models)
          final localFile = File(localPath);
          if (await localFile.exists()) {
            final content = await localFile.readAsString();
            final modelJson = jsonDecode(content);
            _models['${symbol}_$horizon'] = XGBoostModel.fromJson(modelJson);
            print('✅ Loaded local model: $localPath');
            continue;
          }
          ...
          final modelJson = jsonDecode(content);
          _models['${symbol}_$horizon'] = XGBoostModel.fromJson(modelJson);
          print('✅ Loaded asset model: $assetKey');
        } catch (e) {
          print('⚠️ Model not found locally or in assets for ${symbol}_$horizon');
          allLoaded = false;
        }
      }
      return allLoaded;
